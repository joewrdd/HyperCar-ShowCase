    ...
    */
    app.post('/login', async (req, res) => {
        ...
        try {
            ...
            const [users] = await db.query('SELECT * FROM users WHERE name = ? OR email = ?', [name, email]);
            if (users.length === 0) {
            ...
            const token = jwt.sign({ id: user.user_id, name: user.name , role_id: user.role_id},
    ...
    app.get('/admin', authenticateToken, async (req, res) => {
        try {
            ...
            if(AuthenticatedUser && AuthenticatedUser.role_id == 1) {
                res.render('admin', { users, services, manufacturers, reviews, cars, favorites });
            } else{   
                res.status(403).send('Only Admin Users Are Allowed To Access Admin Portal!');
            }
        } catch (err) {
            console.error('Error loading admin panel:', err);
            res.status(500).send('An error occurred while trying to access the admin panel.');
        }
    });
    ...
    */
    app.get('/dashboard', authenticateToken,  async (req, res) => {
        try {
            ...
            res.render('dashboard', { isAuthenticated: isAuthenticated, cars: carsWithImagePaths, reviews: reviews });
        } catch (error) {
    ...
    app.get('/userfavorites', authenticateToken, async (req, res) => {
        try {
            ...
            const favoriteCarsWithImages = favoriteCars.map(favorite => {
                ...
                return {
                    ...favorite,
                    imagePath: car ? `/images/${carImageMapping[car.model] || 'default.jpg'}` : '/images/default.jpg'
                };
            });
    ...
    */
    app.get('/car/:id', authenticateToken, async (req, res) => {
        ...
        try {
            ...
            const [carResult] = await database.query('SELECT *, CONCAT("/images/", images) AS images FROM cars WHERE car_id = ?',
                [carId]); // To get car_id per image since i dont have images in my database from the start.
            const reviews = await reviewService.getReviewByCarId(carId);
            res.render('carDetails', { user: req.user, car: carResult[0], reviews: reviews || [] });
        } catch (err) {
